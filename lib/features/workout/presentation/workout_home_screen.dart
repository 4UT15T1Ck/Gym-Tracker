      ...
      builder: (context, state) {
        return ListView(
          ...
          children: [
              FilledButton.icon(
                onPressed: () async {
                  final id = await context.read<WorkoutHomeCubit>().startEmptyWorkout();
                  if (context.mounted) {
                ...
                children: [
                  Expanded(
                    child: OutlinedButton.icon(
                      onPressed: () async {
                        await Navigator.of(context).pushNamed(Routes.createRoutine);
                        if (context.mounted) context.read<WorkoutHomeCubit>().load();
                      },
                  ...
                  Expanded(
                    child: OutlinedButton.icon(
                      onPressed: () => Navigator.of(context).pushNamed(Routes.exerciseList),
              ...
              Text('Routines (${state.routines.length})', style: Theme.of(context).textTheme.titleMedium),
                  ...
                  child: Column(
                    ...
                    children: [
                      InkWell(
                        onTap: () async {
                          await Navigator.of(context).pushNamed(
                            ...
                            arguments: RoutineDetailRouteArgs(routineId: routine.routine.id),
                          );
                          if (context.mounted) context.read<WorkoutHomeCubit>().load();
                        },
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                children: [
                                  Expanded(
                                    child: Text(routine.routine.name, style: Theme.of(context).textTheme.titleMedium),
                                  ),
                                    ...
                                    onSelected: (value) async {
                                      if (value == 'edit') {
                                        final saved = await Navigator.of(context).pushNamed<bool>(
                                          Routes.editRoutine,
                                          arguments: EditRoutineRouteArgs(detail: routine),
                                        );
                                        ...
                                        if (confirm == true && context.mounted) {
                                          await context.read<WorkoutHomeCubit>().deleteRoutine(routine.routine.id);
                                        }
                                      }
                                    },
                                    itemBuilder: (context) => [
                                      ...
                                      const PopupMenuItem(
                                        value: 'delete', 
                                        child: Text('Delete Routine', style: TextStyle(color: Colors.red)),
                                      ),
                                    ],
                                  ),
                                ],
                      ...
                      Padding(
                        ...
                        child: FilledButton(
                          onPressed: () async {
                            final id = await context.read<WorkoutHomeCubit>().startRoutine(routine);
                            if (context.mounted) {
                              context.read<ShellActiveWorkoutCubit>().refreshNow();
                            }
                            if (!context.mounted) return;
